    ...
    <section id="certifications" className="section-padding bg-white">
      <div className="container-shell">
        ...
        <div className="grid gap-5 md:grid-cols-3">
          {certifications.map(({ title, detail, icon: Icon, link }, index) => (
            <Reveal key={title} delay={index * 0.1}>
              {link ? (
                <a href={link} target="_blank" rel="noopener noreferrer">
                  <article className="h-full rounded-[30px] border border-ink/8 bg-surface p-7 shadow-[0_18px_60px_rgba(10,10,10,0.07)] transition hover:-translate-y-1 hover:border-volt hover:shadow-glow cursor-pointer">
                    <div className="mb-7 flex items-start justify-between">
                      <div className="grid h-14 w-14 place-items-center rounded-2xl bg-carbon text-volt">
                        <Icon className="h-7 w-7" />
                      </div>
                      <ExternalLink className="h-5 w-5 text-volt opacity-0 transition group-hover:opacity-100" />
                    </div>
